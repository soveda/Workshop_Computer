...
The first version is designed as a CV/gate and USB MIDI Eurorack oscillator
with a built-in sting sequencer. Patch it into the Workshop System filters,
Ring Mod, Slopes, Stompbox, or Mix just as you would patch an analog voice.
...
For an instant hands-free version, flip **Z Middle**. The internal sequencer
starts from a 32-step Sandstorm-sting pattern extracted from `sand stings.mid`.
Main sets internal tempo when Pulse In 2 is unpatched; Pulse In 2 clocks the
sequence from rising edges when patched. Pulse In 1 resets the sequence.

Hold **Z Down** for a momentary accent. It forces the envelope open and gives
the voice a small level lift; release it to return to the normal synth or
sequencer behavior.
...
| Z Middle | Sequencer mode: internal or Pulse In 2 clocked sting pattern |
...
Z middle runs a 32-step pattern seeded from the supplied Sandstorm stings MIDI
file:
...
The pattern uses MIDI notes 69, 71, 74, and 76, with rests on the dashes. Main
sets the internal tempo from about 60-240 BPM when Pulse In 2 is unpatched.
Patch a clock to Pulse In 2 to step the sequence externally. Pulse In 1 resets
back to step 1.
