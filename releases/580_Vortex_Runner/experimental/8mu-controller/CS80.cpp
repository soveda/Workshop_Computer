...
private:
    ...
    {
        // The 8mu faders use CC 34-41, which otherwise conflict with this
        // card's generic envelope CCs.  Map them to the existing source/tone
        // controller layout documented for Vortex Runner.
        if (eightMu.Connected() && cc >= 34u && cc < 42u)
        {
            static constexpr uint8_t EightMuFaderCc[EightMU::numFaders] = {
                20u, 21u, 22u, 23u, 24u, 25u, 26u, 27u
            };
            handleMidiControlChange(EightMuFaderCc[cc - 34u], value);
            return;
        }
