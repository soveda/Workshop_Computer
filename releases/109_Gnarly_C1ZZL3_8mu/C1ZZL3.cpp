...
private:
    ...
    // Music Thing 8mu factory faders. These provide direct performance
    // control of all six envelope lanes without changing the existing
    // generic CC20-27 performance map.
    static constexpr uint8_t MidiCc8muAmp1Depth = 34;
    static constexpr uint8_t MidiCc8muPd1Depth = 35;
    static constexpr uint8_t MidiCc8muPitch1Depth = 36;
    static constexpr uint8_t MidiCc8muPitch2Depth = 37;
    static constexpr uint8_t MidiCc8muPd2Depth = 38;
    static constexpr uint8_t MidiCc8muAmp2Depth = 39;
    static constexpr uint8_t MidiCc8muEnvelopeSelect = 40;
    static constexpr uint8_t MidiCc8muEnvelopeMaster = 41;
    static constexpr uint8_t MidiCc8muAmp1Stages = 64;
    static constexpr uint8_t MidiCc8muAmp2Stages = 72;
    static constexpr uint8_t MidiCc8muPd1Stages = 80;
    static constexpr uint8_t MidiCc8muPd2Stages = 88;
    static constexpr uint8_t MidiCc8muPitch1Stages = 96;
    static constexpr uint8_t MidiCc8muPitch2Stages = 104;
    static constexpr uint8_t MidiCc8muSelectEnvelope = 112;
    static constexpr uint8_t MidiCc8muAmp1DepthBank = 113;
    static constexpr uint8_t MidiCc8muPd1DepthBank = 114;
    static constexpr uint8_t MidiCc8muPitch1DepthBank = 115;
    static constexpr uint8_t MidiCc8muPitch2DepthBank = 116;
    static constexpr uint8_t MidiCc8muPd2DepthBank = 117;
    static constexpr uint8_t MidiCc8muAmp2DepthBank = 118;
    static constexpr uint8_t MidiCc8muMasterDepthBank = 119;
    static constexpr uint8_t MidiCc8muEditLevels = 120;
    static constexpr uint8_t MidiCc8muEditTimes = 121;
    static constexpr uint8_t MidiCc8muSaveEnvelope = 122;
    static constexpr uint8_t MidiCc8muRevertEnvelope = 123;
    ...
    {
        updateEnvelope();
        int32_t amp1Level = scaleAmplitudeEnvelope(ampEnvelopeLevel, envelopeAmp1Depth);
        int32_t amp2Level = scaleAmplitudeEnvelope(amp2EnvelopeLevel, envelopeAmp2Depth);
        int32_t pd1Level = scaleModulationEnvelope(pdEnvelopeLevel, envelopePd1Depth);
        int32_t pd2Level = scaleModulationEnvelope(pd2EnvelopeLevel, envelopePd2Depth);
        int32_t pitch1Level = scalePitchEnvelope(pitchEnvelopeLevel, envelopePitch1Depth);
        int32_t pitch2Level = scalePitchEnvelope(pitch2EnvelopeLevel, envelopePitch2Depth);

        int32_t freq1 = applyPitchEnvelopeToFrequency(freq, pitch1Level);
        int32_t pd1 = applyEnvelopeToPd(pd1Base, pd1Level);
        int32_t pd2 = applyEnvelopeToPd(pd2Base, pd2Level);
        ...
        int32_t ampScale1 = (envelopeAmpScale(amp1Level) * sharedScale) >> 12;
        int32_t ampScale2 = (envelopeAmpScale(amp2Level) * sharedScale) >> 12;
    ...
    int32_t scaledEnvelopeDepth(int32_t depth)
    {
        return (clamp12(depth) * clamp12(envelopeMasterDepth)) >> 12;
    }

    int32_t scaleAmplitudeEnvelope(int32_t level, int32_t depth)
    {
        return (clamp12(level) * scaledEnvelopeDepth(depth)) >> 12;
    }

    int32_t scaleModulationEnvelope(int32_t level, int32_t depth)
    {
        return (clamp12(level) * scaledEnvelopeDepth(depth)) >> 12;
    }

    int32_t scalePitchEnvelope(int32_t level, int32_t depth)
    {
        int32_t scaledOffset =
            ((clamp12(level) - (int32_t)PitchEnvelopeCenter) *
             scaledEnvelopeDepth(depth)) >> 12;
        return clamp12((int32_t)PitchEnvelopeCenter + scaledOffset);
    }
    ...
    bool activeCustomEnvelopeSlot(uint8_t& slot)
    {
        if (envelopePreset < CustomEnvelopePreset)
            return false;
        slot = envelopePreset - CustomEnvelopePreset;
        return slot < CustomEnvelopeSlotCount && customEnvelopePersist[slot];
    }

    void edit8muEnvelopeStage(uint8_t cc, uint8_t value)
    {
        uint8_t slot = 0;
        if (!activeCustomEnvelopeSlot(slot))
            return;

        EnvelopeStage* lane = nullptr;
        uint8_t stage = cc & 0x07u;
        if (cc >= MidiCc8muAmp1Stages && cc < MidiCc8muAmp1Stages + 8u)
            lane = customEnvelopes[slot].amp;
        else if (cc >= MidiCc8muAmp2Stages && cc < MidiCc8muAmp2Stages + 8u)
            lane = customEnvelopes[slot].amp2;
        else if (cc >= MidiCc8muPd1Stages && cc < MidiCc8muPd1Stages + 8u)
            lane = customEnvelopes[slot].pd;
        else if (cc >= MidiCc8muPd2Stages && cc < MidiCc8muPd2Stages + 8u)
            lane = customEnvelopes[slot].pd2;
        else if (cc >= MidiCc8muPitch1Stages && cc < MidiCc8muPitch1Stages + 8u)
            lane = customEnvelopes[slot].pitch;
        else if (cc >= MidiCc8muPitch2Stages && cc < MidiCc8muPitch2Stages + 8u)
            lane = customEnvelopes[slot].pitch2;

        if (lane == nullptr)
            return;

        if (eightMuEditTimes)
            lane[stage].time = 1u + (((uint32_t)value * MaxWebMidiStageSamples) / 127u);
        else
            lane[stage].level = (uint16_t)midiCcToControl(value);
    }

    void save8muEnvelope()
    {
        uint8_t slot = 0;
        if (!activeCustomEnvelopeSlot(slot))
            return;
        customEnvelopeSaved[slot] = customEnvelopes[slot];
        customEnvelopeSavedPerformances[slot] = currentSlotPerformanceState();
        saveCustomEnvelopeState();
    }

    void revert8muEnvelope()
    {
        uint8_t slot = 0;
        if (activeCustomEnvelopeSlot(slot))
            customEnvelopes[slot] = customEnvelopeSaved[slot];
    }
    ...
    void processMidiVoiceByte(uint8_t byte)
    {
        ...
        if (type == 0xB0u)
        {
            if ((midiData[0] >= MidiCc8muAmp1Stages && midiData[0] < MidiCc8muAmp1Stages + 8u) ||
                (midiData[0] >= MidiCc8muAmp2Stages && midiData[0] < MidiCc8muAmp2Stages + 8u) ||
                (midiData[0] >= MidiCc8muPd1Stages && midiData[0] < MidiCc8muPd1Stages + 8u) ||
                (midiData[0] >= MidiCc8muPd2Stages && midiData[0] < MidiCc8muPd2Stages + 8u) ||
                (midiData[0] >= MidiCc8muPitch1Stages && midiData[0] < MidiCc8muPitch1Stages + 8u) ||
                (midiData[0] >= MidiCc8muPitch2Stages && midiData[0] < MidiCc8muPitch2Stages + 8u))
            {
                edit8muEnvelopeStage(midiData[0], midiData[1]);
            }
            else if (midiData[0] == MidiCc8muEditLevels && midiData[1] > 0u)
            {
                eightMuEditTimes = false;
            }
            else if (midiData[0] == MidiCc8muEditTimes && midiData[1] > 0u)
            {
                eightMuEditTimes = true;
            }
            else if (midiData[0] == MidiCc8muSaveEnvelope && midiData[1] > 0u)
            {
                save8muEnvelope();
            }
            else if (midiData[0] == MidiCc8muRevertEnvelope && midiData[1] > 0u)
            {
                revert8muEnvelope();
            }
            else if (midiData[0] == MidiCc8muSelectEnvelope)
            {
                uint8_t count = selectableEnvelopeCount();
                envelopePreset = envelopePresetForSelection((uint8_t)(((uint16_t)midiData[1] * count) >> 7));
            }
            else if (midiData[0] == MidiCc8muAmp1Depth || midiData[0] == MidiCc8muAmp1DepthBank)
            {
                envelopeAmp1Depth = midiCcToControl(midiData[1]);
            }
            else if (midiData[0] == MidiCc8muPd1Depth || midiData[0] == MidiCc8muPd1DepthBank)
            {
                envelopePd1Depth = midiCcToControl(midiData[1]);
            }
            else if (midiData[0] == MidiCc8muPitch1Depth || midiData[0] == MidiCc8muPitch1DepthBank)
            {
                envelopePitch1Depth = midiCcToControl(midiData[1]);
            }
            else if (midiData[0] == MidiCc8muPitch2Depth || midiData[0] == MidiCc8muPitch2DepthBank)
            {
                envelopePitch2Depth = midiCcToControl(midiData[1]);
            }
            else if (midiData[0] == MidiCc8muPd2Depth || midiData[0] == MidiCc8muPd2DepthBank)
            {
                envelopePd2Depth = midiCcToControl(midiData[1]);
            }
            else if (midiData[0] == MidiCc8muAmp2Depth || midiData[0] == MidiCc8muAmp2DepthBank)
            {
                envelopeAmp2Depth = midiCcToControl(midiData[1]);
            }
            else if (midiData[0] == MidiCc8muEnvelopeSelect)
            {
                uint8_t count = selectableEnvelopeCount();
                uint8_t selection =
                    (uint8_t)(((uint16_t)midiData[1] * count) >> 7);
                envelopePreset = envelopePresetForSelection(selection);
            }
            else if (midiData[0] == MidiCc8muEnvelopeMaster || midiData[0] == MidiCc8muMasterDepthBank)
            {
                envelopeMasterDepth = midiCcToControl(midiData[1]);
            }
            else if (midiData[0] == MidiCcModWheelOsc1Pd || midiData[0] == MidiCcOsc1Pd)
            {
...
private:
    ...
    int32_t osc2Ring = 0;
    int32_t osc2Noise = 0;
    // These default to full depth, so the card behaves exactly like Gnarly
    // until an 8mu fader is moved. MIDI runs on core 1; aligned 32-bit reads
    // and writes are atomic on RP2040, and volatile keeps these live values
    // visible to the audio core.
    volatile int32_t envelopeAmp1Depth = 4095;
    volatile int32_t envelopePd1Depth = 4095;
    volatile int32_t envelopePitch1Depth = 4095;
    volatile int32_t envelopePitch2Depth = 4095;
    volatile int32_t envelopePd2Depth = 4095;
    volatile int32_t envelopeAmp2Depth = 4095;
    volatile int32_t envelopeMasterDepth = 4095;
    volatile bool eightMuEditTimes = false;
